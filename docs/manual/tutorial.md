...
Click the node at 25, 12 again and look at the **Node** panel on the right. The
`Corner` / `Smooth` / `Symm` buttons show which one is lit. It reads **Corner**,
because one handle exists and the other does not.
...
Press **Download**. The file saves to disk. Open it in a browser to check.
