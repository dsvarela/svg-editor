...
| `[~]` | **Offset path** — done for convex geometry, as **Offset** in the Draw panel. Samples the true offset and fits cubics through it, which works because a parallel curve shares its original's tangent direction and `core/fit.ts` takes the end tangents as inputs: a circle of radius 20 offset by 5 is within 0.042 everywhere, in ten nodes. **Open:** on the inside of a corner the neighbouring offsets overrun, the sampled polyline doubles back, and the fit through it leaves the offset by as much as the distance asked for. Measured and bounded in `test/offset.test.ts` rather than described. See ARCHITECTURE §39. | Illustrator, Inkscape | L |
| `[ ]` | **Stroke to path** — convert a stroked outline into a filled shape. Blocked on the overrun above, not on the offsetting: the two offsets of a stroke always meet at the caps, so it needs the topology answer that Offset path does without. Two routes tried and recorded in ARCHITECTURE §39 -- path-bool throws on a self-union, and splicing out the crossings left a stray piece at the corner. | Boxy SVG, Inkscape | L |
