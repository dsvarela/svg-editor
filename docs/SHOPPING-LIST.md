...
| `[ ]` | **Auto-smooth node** — built, then removed on 2026-08-20. Handles derived from the chord between the neighbours, re-derived whenever anything nearby moved. It worked and nothing on the canvas ever said which nodes were auto, so an edit rewrote geometry the person had not touched with no way to see which. Anything bringing it back needs a mark on the node before it needs a button. See ARCHITECTURE §65. | Inkscape | — |
...
| `[-]` | **A style on a group** — decided against, 2026-08-19, and the entry's own condition is why: it was worth doing "only with an answer to which of the two wins when they disagree", and every answer is a rule the panel would have to teach. Keeping a fill on the group means a shape's painted colour is not a property of the shape, which is §5's objection to a stored transform and §6's to a stored node type a third time. An imported `<g fill>` is pushed down onto the shapes and stays that way. The operation people want already works, because selecting a group selects its shapes; what is refused is the inheritance. §61. | SVG | S |
| `[x]` | **Entering a group** — done as the half that was missing, and not as Illustrator's mode. A group's shapes stay individually selectable, which is what lets one be nudged without leaving the group; **Select group** (`Shift+G`) is the way back up, one level per press. The level is read off the selection -- the nearest ancestor group not already wholly selected -- rather than stored, so nothing is remembered between presses and a selection made by hand behaves like the same one arrived at by pressing. §61. | Illustrator | S |
...
| `[x]` | **Import an SVG file** — done, as **Add an SVG** in the Document panel, and by dropping one on the canvas. The same importer as pasting into the source drawer: the same refusal of anything that draws nothing, one undo step. What it adds is the reading. Asked for 2026-08-13; `applySource` was split so the file and the box share everything after the text arrives. It replaced the document until 2026-08-19, when it was changed to add to it: a file is something you brought to a drawing you are already working on, and the source drawer is where "the document is now this" belongs. The canvas keeps its own size for the same reason. | every editor | S |
...
| `[x]` | **Round a corner from a node** — done. **Radius** and **Round** in the Node panel replace the selected corners with an arc of that radius, tangent to both sides, which is the operation the rectangle tool performs only while drawing. Either side may be a curve: what decides is whether the path turns at the node, and the arc is solved against whatever the sides are rather than approximated near them. A fillet a fraction of a degree off tangent looks right and is wrong, so it is placed exactly. The radius is clamped to what the shorter side holds, and the clamp is reported. Applied from the highest index down, since each rounded corner turns one node into two. See ARCHITECTURE §23. | Figma corner radius, Illustrator live corners | — |
...
| `[x]` | **Export PNG at a chosen size** — done, as **Download PNG** in the **File** group: a button and a width. The height follows the canvas proportions rather than the drawing, so padding somebody chose is kept. Needed no dependency and no server, because a data-URI SVG does not taint a canvas and so can be read back; §53 has the argument, and the same string feeds the previews below. The browser scenario reads the bytes back and counts the opaque pixels, because a well-formed PNG of nothing has a valid header too. | — | S |
| `[x]` | **Target-size preview** — done, as the **Preview** group: the drawing at 16, 24, 32 and 48 pixels, on a chequer, each shaped like the canvas. Drawn from the exported SVG, so File's Output settings show in it and rounding the decimals can be watched spoiling a curve before a file is saved with it. Held still during a drag and while the group is shut, because pointing four `<img>` elements at a fresh data URI is four rasters of the whole document. | icon-design practice | S |

## Smaller things, and one that needs a decision

| | Feature | Source | Size |
|---|---|---|---|
| `[x]` | **Survive a refresh** — done. The session is written to `localStorage` on a timer and read back on load, and the header beside **On this device** says whether that is happening rather than showing a tick with nothing behind it. It refuses for two reasons and names both: a page opened from `file://` gets an origin Chromium attaches no storage to, and a drawing past two megabytes of text is more than a page's quota. Both point at a workspace file, which has neither limit. What to save turned out to be the same question as the item below, so one reader answers both. §59. | — | S |
| `[x]` | **A workspace file** — done, as **Workspace: Save** and **Open** in the File group. The drawing, the camera, the guides, the saved styles and every switch, as one JSON file. Opening one replaces the session, which is why it is not filed under Add: two cameras is not a camera. The reader rebuilds every object field by field and returns a sentence rather than throwing, because the same reader runs at startup on whatever is in storage and a restore that threw would brick the editor until somebody cleared it by hand. The backdrop image does not go in, and the message says so. §59. | — | M |
| `[x]` | **Star and polygon tools** — done, as one tool with a **Polygon** group beside the style for new shapes: a mode toggle, a corner count and a star's inner ratio. It added a generator and nothing to the model, which is what the entry predicted: every node comes out with no handles, so a polygon is roundable at every corner and stays straight-sided under later edits. The shape is inscribed in the drag rather than stretched to fill it, and the first corner is at the top, because a star turned a tenth of a turn reads as a wrong star. §63. | Inkscape, Boxy SVG | S |
| `[x]` | **Repeat the last transform** — done, as **Repeat** and `Shift`+`T`, and not `Ctrl`+`D`, which Duplicate already holds and presses first. Cheap for the reason predicted: §5 bakes transforms into coordinates, so the last matrix is the whole of what has to be remembered. What it remembers is the matrix rather than the gesture, which is the same thing for a rotate and deliberately not for a typed width, so the readout beside the button names which one you have. A box drag and an arrow nudge both count. §62. | Illustrator | S |
| `[x]` | **Booleans between the paths of one shape** — done. Select two paths of one shape in the list and the same four operations combine those, leaving the shape and the paths you did not select alone. It needed its own entry point rather than a flag on `booleanShapes`, which the entry half predicted: that one concatenates a shape's subpaths into one region, which is what makes a hole a hole, and this hands each over as a region of its own. Opposite readings of the same geometry, so neither can be the other with an argument. A shape selected whole still declines, because that is a question about shapes. §64. | — | M |
| `[x]` | **Opacity** — done, as a percentage in the **Style** group, stored as SVG's 0 to 1 and written only when it is below 1, since `opacity="1"` is the initial value. The objection stands and is answered by stopping at one number: `fill-opacity` and `stroke-opacity` are refused, because three that multiply is a compositing rule rather than a fact about the drawing, and a colour with alpha in it goes the same way. Reading is the direction where all three have to be reconciled, and an `opacity` on a `<g>` is multiplied into the shapes under it, because a group carries no style here. §60. | SVG | S |
