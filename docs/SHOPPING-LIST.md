...
| `[~]` | **Offset path** — done bar one case, as **Offset** in the Draw panel. Samples the true offset and fits cubics through it, which works because a parallel curve shares its original's tangent direction and `core/fit.ts` takes the end tangents as inputs. The overrun at a concave corner is removed by the distance criterion before fitting -- Chen & McMains's rule that the invalid parts bound regions of non-positive winding number, in its local form -- so a 40-unit square inward by 4 is now exact to 0.0006 where it used to be 4.0 out, an offset that consumes its shape returns nothing, and one that separates returns the pieces. **Open:** a curve fitted between two valid samples can still bulge through the invalid space between them, up to 1.14 on an eight-unit offset. See ARCHITECTURE §39. | Illustrator, Inkscape | L |
| `[ ]` | **Stroke to path** — convert a stroked outline into a filled shape. The offsetting is there and correct except for one case, and that case is the one this would live in: the two offsets of a stroke meet at every cap, which is exactly where a fitted curve can still bulge through invalid space. Needs a validation pass over the fitted curves, refitting wherever the distance criterion fails. See ARCHITECTURE §39. | Boxy SVG, Inkscape | L |
