...
| `[ ]` | **Round a corner from a node** — take an existing corner node and replace it with a fillet arc of a given radius, the operation the rectangle tool's **Corner** performs only at draw time. Asked for 2026-08-12. The natural sibling of Break and Fuse, and the one corner operation this editor has no answer for. | Figma corner radius, Illustrator live corners | M |
