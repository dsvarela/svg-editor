...
| `[~]` | **Overlay at low zoom** — measured 2026-08-11, and the dash theory in the first version of this entry was wrong: `vector-effect: non-scaling-stroke` already keeps the selection box a constant width on screen. What actually goes wrong is crowding, since every marker is screen-sized while the drawing shrinks. Handles nearer their node than the node's own marker are now dropped, which clears the worst of it below about 120 px, but the anchors themselves still pile up. Reproducing the reported view exactly is the open part. | — | S |
