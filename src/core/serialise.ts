...
import { segmentAsCubic, segmentCount, segmentIsLine } from './types';
 ...
 */
function writeCommands(cmds: Cmd[], decimals: number, minify: boolean): string {
  if (!minify) {
    return cmds
      .map((c) => (c.args.length ? `${c.letter} ${c.args.map((a) => formatNumber(a, decimals)).join(' ')}` : c.letter))
      .join(' ');
  }
...
/** Serialise one subpath. */
function serialiseSubpath(sp: Subpath, opts: Required<SerialiseOptions>, eps: number): Cmd[] {
  ...
  cmds.push({ letter: 'M', args: [start[0], start[1]] });
  ...
  for (let i = 0; i < nSeg; i++) {
    ...
    if (segmentIsLine(sp, i)) {
      ...
      if (qv(end[1]) === qv(cur[1])) {
        cmds.push({ letter: 'H', args: [end[0]] });
      } else if (qv(end[0]) === qv(cur[0])) {
        cmds.push({ letter: 'V', args: [end[1]] });
      } else {
        cmds.push({ letter: 'L', args: [end[0], end[1]] });
      }
      prevC2 = null;
    } else {
      const q = shorthands ? cubicAsQuad(c, eps) : null;
      if (q) {
        cmds.push({ letter: 'Q', args: [q[0], q[1], end[0], end[1]] });
        prevC2 = null;
      } else {
        const mirrored =
          shorthands &&
        ...
        if (mirrored) {
          cmds.push({ letter: 'S', args: [c[2][0], c[2][1], end[0], end[1]] });
        } else {
          cmds.push({ letter: 'C', args: [c[1][0], c[1][1], c[2][0], c[2][1], end[0], end[1]] });
        }
        prevC2 = [c[2][0], c[2][1]];
      }
    }
...
function preferRelative(cmds: Cmd[], decimals: number): Cmd[] {
  ...
  for (const c of cmds) {
    ...
    out.push(useRel ? { letter: c.letter.toLowerCase(), args: rel } : c);
...
export function serialisePath(subpaths: Subpath[], options: SerialiseOptions = {}): string {
  ...
  const cmds = subpaths
    ...
    .flatMap((sp) => serialiseSubpath(sp, opts, eps));

  return writeCommands(cmds, opts.decimals, opts.minify);
}
